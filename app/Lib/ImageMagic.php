...
class ImageMagic
{
    ...
    protected $cacheDir = '/files/cache/images';
    protected $actualCacheDir = '/app/webroot/files/cache/images';
    ...
    public function __construct($path = NULL, $width = NULL, $height = NULL)
    {
        ...
        if (!(extension_loaded('imagick'))) {
            throw new \RuntimeException('You need to install Imagick PHP Extension OR use http://github.com/Gregwar/ImageMagic library');
        }
    ...
    public function generateFileFromHash($hash)
    {
        ...
        if ($this->actualCacheDir === NULL) {
            ...
            $actualDirectory =  $_SERVER['DOCUMENT_ROOT'].$this->actualCacheDir;
        }
    ...
    public function save($file, $type = 'jpg', $quality = 80)
    {
        ...
        if ($type == 'GIF' || $type == 'gif') {
            ...
            return $this->image->writeImages( $file . 'gif', TRUE) === TRUE;
        }

        $this->image->setImageFormat($type);
        $file = preg_replace('/[^\.]*$/', '', $file);
        $this->image->writeImage( $file . strtolower($type));
