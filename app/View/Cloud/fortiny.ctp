<?php
   ...
   $controller = array(
    ...
    $cloudManagerView = $view ? $view : 'middle';
...
<?
    ...
    echo '<a href="/Cloud/fortiny" class="tooltip-select'.(isset($id) ? '' : ' active').'">'.$icon.__('My files')."</a>";
    foreach($folders as $row) {
        $class = $row['Cloud']['id'] == $id ? ' active' : '';
        $icon = ((isset($id) && $row['Cloud']['id'] == $id) ? '<span class="glyphicons chevron-right"></span>' : '');
        if(!$tree) {
            echo '<a href="/Cloud/fortiny/'.$row['Cloud']['id'].'" class="tooltip-select'.$class.'">'.$icon.str_repeat('- ',count($tree)+1).$row['Cloud']['name']."</a>";
    ...
    echo '<a href="/Cloud/fortiny/shared" class="tooltip-select'.((strpos($_SERVER['REQUEST_URI'], 'shared') == false) ? '' : ' active').'">'.$icon.__('Shared with me')."</a>";
?>
</div>
...
<div class="fileStorage clearfix">
    <div class="settings clearfix">
		...
		<a href="<?=$this->Html->url(array('controller' => "cloud", 'action' => "usage"))?>" class="progress btn-group usage-progress" style="float: right;">
...
$(document).ready(function(){
    ...
    var $pswp = $('.pswp')[0];
	$('.foldersAndFiles .item:not(".shared-folder")').off('click');
	$('.foldersAndFiles .item:not(".shared-folder")').on('click', function (e) {

		$('.foldersAndFiles .item.active').removeClass('active');
