...
<?php
	...
	$this->Html->addCrumb(__('Subscriptions'), array('controller' => 'Article', 'action' => 'subscriptions'));
?>
...
<?php
	if ($aArticles) {
		...
		foreach($aArticles as $article) {
			...
			$aContainer[$i].= $this->element('article_entry', array('article' => $article, 'comments' => count($comments), 'category' => true, 'newArticle' => strtotime($currUser['User']['news_update'])<strtotime($article['Article']['created']) ));
			$i++;
			if ($i >= 3) {
				$i = 0;
			}
