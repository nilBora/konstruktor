...
<form id="finance-save-goal">
	<input type="hidden" name="FinanceGoal[project_id]" value="<?= $id ?>" required="true">
	<div class="form-group">
		<label><?= __('Goal name') ?></label>
		<input type="text" name="FinanceGoal[name]" value="" placeholder="<?= __('Goal') ?>" class="form-control" required="true">
	</div>
	...
	<div class="form-group">
		...
		<input type="number" step="0.01" name="FinanceGoal[final_sum]" class="form-control" placeholder="<?= __('Amount required') ?>" required="true">
	</div>
	<div class="clearfix">
...
$('#finance-add-goal-finish').datetimepicker('setDate', new Date());
...
$('#finance-save-goal').on('submit', function () {
	...
	$.post(financeURL.addGoal + '/<?= $id ?>', data, function (response) {
		if (response) {
			alert(response);
			return;
		}
