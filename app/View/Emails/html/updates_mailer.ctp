...
<?
					switch( $key ) {
						case 'UserEvent':
						case 'UserEventShare':
							$aUserID = explode(',', $event['UserEvent']['recipient_id']);
							$eUsers = array();
							//$eUser = $data['users'][$event['UserEvent']['recipient_id']];
							if($event['UserEvent']['user_id'] != $userID) {
								$eUsers[] = $data['users'][$event['UserEvent']['user_id']];
							} else {
								foreach($aUserID as $uid) {
									$eUsers[] = $data['users'][$uid];
								}
							}
?>
						<div style="border-radius: 4px; padding: 6px 10px;">
							...
							<div class="text" style="display: block; margin-left: 65px;">
								You have an event:
								<div class="title" style="display: inline-block;" style="font-weight: 600"><?=$event['UserEvent']['title']?></div>
								<div class="additionalBlock" style="padding-top: 8px; margin-top: 8px; border-top: 1px solid #C4C4C4;">
<?php
				foreach($eUsers as $user) {
?>
									<div class="clearfix" style="margin: 4px 0; font-size: 15px;">
										<img class="ava" src="<?=$hostname.$this->Media->imageUrl($user['UserMedia'], 'thumb50x50')?>" alt="<?=$user['User']['full_name']?>" onclick="window.location.href='/User/view/71'" style="margin-left: 42px; width: 25px; height: 25px;">
										<a class="userLink" href="/User/view/<?=$user['User']['id']?>" style="display: inline-block; margin-left: 4px; line-height: 24px; height: 24px; color: #1580be; text-decoration: underline;"><?=$user['User']['full_name']?></a>
									</div>
<?php
				}
?>
...
<?php
				if($event['UserEvent']['place_name']) {
?>
								<div class="additionalBlock" style="padding-top: 8px; margin-top: 8px; border-top: 1px solid #C4C4C4; font-size: 13px; color: #616161;">
									<div class="description eDescr">Event place:</div>
									<div class="description eDescr" style="font-style: italic;"><?=$event['UserEvent']['place_name']?></div>
									<img src="https://maps.googleapis.com/maps/api/staticmap?center=<?=$event['UserEvent']['place_coords']?>&amp;zoom=14&amp;scale=2&amp;size=400x200&amp;markers=size:mid%7Ccolor:0x22b5ae%7Clabel:no%7C<?=$event['UserEvent']['place_coords']?>" alt="<?=$event['UserEvent']['place_name']?>" style="width: 100%; margin-top: 10px; margin-bottom: 10px; box-shadow: 0 2px 3px 2px rgba(0, 0, 0, .25);">
								</div>
<?php
				}
?>
...
<?
						...
						case 'Article':
							...
							$article = $data['articles'][$event['Article']['created']];
?>
						<div style="border-radius: 4px; padding: 6px 10px;">
							<div class="eventTime" style="float: left; font-size: 11px; color: #414141; font-style: italic; display: inline-block; vertical-align: middle; margin-top: 5px;"><?= date( 'H:i', $created ) ?></div>
							...
							<div class="additionalBlock" style="padding-top: 8px; margin-top: 8px; border-top: 1px solid #C4C4C4; font-size: 13px; color: #616161;">
								<img class="ava" style="width: 50px; height: auto; float: left; display: block; margin-left: 42px; margin-bottom: 8px;" src="<?=$hostname.$this->Media->imageUrl($article['ArticleMedia'], 'thumb100x100')?>" alt="<?=$eGroup['Group']['title']?>" />
								<div class="articleText" style="display: block; margin-left: 110px; margin-right: 0;">
									<a href="<?=$url?>" style="line-height: 24px; height: 24px; color: #1580be; text-decoration: underline;"><?=$event['Article']['title']?></a>
								</div>
								<div style="clear: both;"></div>
							</div>
						</div>
