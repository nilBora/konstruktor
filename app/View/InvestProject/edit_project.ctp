<?php
	...
	$this->Html->addCrumb(__('Edit project'), array('controller' => 'InvestProject', 'action' => 'editProject'));
...
<h1><?= __('Investment project settings') ?></h1>
<div class="row investmentsCreate">
	<form id="invest-project-create" method="post">
		<input type="hidden" name="InvestProjectAvatar[id]" value="<?= $aInvestProject['Avatar']['id'] ?>" id="invest-project-avatar"/>
		...
		<div class="col-sm-3 leftFormBlock">
			...
			<img src="<?= $this->Media->imageUrl($aInvestProject['Avatar'], 'thumb200x150') ?>" alt="" class="img-responsive" id="invest-project-avatar-img"/>
			<input type="file" class="filestyle" id="invest-project-avatar-uploader"
				   ...
				   data-object_id="<?= $aInvestProject['InvestProject']['id'] ?>"
				   accept="image/*"
				/>
		...
		<div class="col-sm-9">
			<div class="form-group">
				...
				<input name="InvestProject[name]" value="<?= $aInvestProject['InvestProject']['name'] ?>"  data-label="<?= __('Project name') ?>" required="true" type="text" class="form-control" />
			</div>
			<div class="form-group">
				<label><?= __('Announcement project') ?></label>
				<input name="InvestProject[note]" value="<?= $aInvestProject['InvestProject']['note'] ?>" data-label="<?= __('Announcement project') ?>"  required="true" type="text" class="form-control" />
			</div>
			<select name="InvestProject[category_id]" class="category">
				<? foreach ($aInvestCategory as $item) { ?>
					<option value="<?= $item['InvestCategory']['id'] ?>"
						<? if ($aInvestProject['InvestProject']['category_id'] == $item['InvestCategory']['id']) {?>selected="selected" <? } ?>
					>
						<?= $item['InvestCategory']['title'] ?>
					</option>
				<? } ?>
			</select>
		</div>
		<div class="col-sm-12">
			<div class="form-group">
				...
				<div class="input-group">
					...
					<input name="InvestProject[video]" value="<?= $aInvestProject['InvestProject']['video'] ?>" class="form-control" placeholder="" type="text">
				</div>
			</div>
			...
			<?=$this->Redactor->redactor('body', array('value' => $aInvestProject['InvestProject']['body']))?>
		</div>
		<div class="col-sm-4">
			<div class="form-group">
				<label><?= __('Necessary sum') ?></label>
				<div class="needSum">
					<input name="InvestProject[total]" value="<?= $aInvestProject['InvestProject']['total'] ?>" type="number" step="0.01"  data-label="<?= __('Necessary sum') ?>" required="true" class="form-control" placeholder="" />
                    <select name="InvestProject[currency]" class="currency">
                        <option value="USD" <? if ($aInvestProject['InvestProject']['currency'] == 'USD') {?>selected="selected" <? } ?>><?= $this->Money->symbolFor('USD') ?></option>
                        <option value="EUR" <? if ($aInvestProject['InvestProject']['currency'] == 'EUR') {?>selected="selected" <? } ?>><?= $this->Money->symbolFor('EUR') ?></option>
                        <option value="RUS" <? if ($aInvestProject['InvestProject']['currency'] == 'RUB') {?>selected="selected" <? } ?>><?= $this->Money->symbolFor('RUB') ?></option>
                    </select>
				</div>
			</div>
		</div>
		<div class="col-sm-4 col-sm-offset-1">
			<div class="form-group">
				...
				<div class="input-group">
					...
					<input name="InvestProject[duration]" value="<?= $aInvestProject['InvestProject']['duration'] ?>" data-label="<?= __('Project duration') ?>" required="true" type="number" min="1" max="60" step="1" class="form-control" placeholder="" />
				</div>
			</div>
		</div>
		...
		<div class="col-sm-9 reward" id="invest-project-reward-list">
		<?
		$rewardIndex = 0;
		foreach ($aInvestProject['Rewards'] as $item) {
		?>
			<div class="back" style="margin-bottom: 4px">
				<input name="InvestReward[<?= $rewardIndex ?>][id]" class="form-control" value="<?=$item['id']?>" type="hidden"/>
                <span id="reward-remove-<?=$item['id']?>" class="glyphicons remove"></span>
				<div class="form-group">
					<label><?= __('Title') ?></label>
					<input name="InvestReward[<?= $rewardIndex ?>][name]" value="<?=$item['name']?>" type="text" class="form-control" data-label="<?= __('Title') ?>" required="true" />
				</div>
				<div class="form-group">
					<label><?= __('Description') ?></label>
					<input name="InvestReward[<?= $rewardIndex ?>][description]" value="<?=$item['description']?>" type="text" class="form-control" data-label="<?= __('Description') ?>" required="true" />
				</div>
				<div class="row">
					<div class="col-sm-6">
						<div class="form-group">
							...
							<div class="input-group">
								<input name="InvestReward[<?= $rewardIndex ?>][total]" value="<?=$item['total']?>" type="number" step="0.01" class="form-control" data-label="<?= __('Amount') ?>"  required="true"/>
							</div>
						</div>
					</div>
					<div class="col-sm-6">
						<div class="form-group">
							...
							<div class="input-group">
								...
								<input type="text" class="form-control" data-label="<?= __('Delivery date') ?>" required="true" id="invest-project-reward-<?=$rewardIndex?>" readonly />
								<input name="InvestReward[<?= $rewardIndex ?>][created]" value="<?=$item['created']?>" type="hidden" id="invest-project-reward-<?=$rewardIndex?>-mirror" class="form-control" data-label="<?= __('Delivery date') ?>" required="true"/>
							</div>
						</div>
					</div>
				</div>
			...
			<script type="text/javascript">
				$('#invest-project-reward-<?=$rewardIndex?>').datetimepicker({
					format: '<?= $dateFormat?>',
					weekStart: 1,
					autoclose: 1,
					todayHighlight: 1,
					...
					minView: 2,
					language:"<?=$lang?>",
					linkField: 'invest-project-reward-<?=$rewardIndex?>-mirror',
					linkFormat: 'yyyy-mm-dd hh:ii:ss'
				});
				$('#invest-project-reward-<?=$rewardIndex?>').datetimepicker('setDate', new Date('<?=$item['created']?>'));

				$('#invest-project-reward-<?=$rewardIndex?>').datetimepicker().on('focus', function () {
				...
				$('#reward-remove-<?=$item['id']?>').on('click', function () {
					$.post(investURL.removeReward + '/<?=$item['id']?>');
...
// Pre-load
	var investProjectGalleryInit = [];
	<? foreach ($aInvestProject['GalleryImages'] as $item) { ?>
	investProjectGalleryInit.push({
		type: 'image',
		image: '<?= $this->Media->imageUrl($item, 'thumb100x100') ?>',
		id: <?= $item['id'] ?>
	});
	<? } ?>
	<? foreach ($aInvestProject['GalleryVideos'] as $item) { ?>
	investProjectGalleryInit.push({
		type: 'video',
		image: 'http://img.youtube.com/vi/<?= $item['youtube_id'] ?>/default.jpg',
...
// Gallery
	...
	investProjectGallery.images = investProjectGalleryInit;
	investProjectGallery.render();
	...
	$('#invest-project-gallery-uploadImage').fileupload({
		...
		dataType: 'json',
			...
			$.post(mediaURL.move, file, function (response) {
				...
				image = image.replace('100x100', 'thumb100x100');
				var id = response.data[0].Media.id;
...
// Reward's
	var investProjectRewardCounter = <?= $rewardIndex ?>;
