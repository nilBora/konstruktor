...
<div class="row baseInfoRow">
	...
	<div class="col-sm-4 leftFormBlock">
		<?
		if ($id) {
			?>
	...
	<div class="col-sm-8 rightFormBlock">
		...
		<div class="form-group noBorder">
			...
			<div class="input-group">
				<?=$this->Form->input('Group.cat_id', array('id'=> 'buy','options' => $eGroupCategories, 'label' => false, 'empty' => __('Select category'), 'class' => 'formstyler countrySelect' ))?>
			</div>
		</div>
		...
		<div class="form-group noBorder">
			...
			<div class="input-group">
				<?=$this->Form->input('Group.responsible_id', array('id'=> 'group-responsible','options' => $eMembers, 'label' => false, 'empty' => __('Select Group Responsible'), 'data-placeholder' => __('Select Group Responsible'), 'class' => 'formstyler responsibleSelect' ))?>
			</div>
		</div>
	...
	<div class="col-sm-9 rightFormBlock">
		...
		if ($aGroupAddress) {
			foreach($aGroupAddress as $i => $groupAddress) {
				if(!array_key_exists($groupAddress['country'],$eMainCountries)){
					$eMainCountries[$groupAddress['country']] = $eAllCountries[$groupAddress['country']];
				}
			}
		}
		asort($eMainCountries);
		$eMainCountries['more'] = "-- [".__('Show More')."] --";
				...
				<div id="addrBlock_<?=$i?>" class="group-fieldset addressBlock">
					...
					<div class="form-group noBorder">
						...
						<div class="input-group input-large">
							...
							<?=$this->Form->input('GroupAddress.'.$i.'.country', array('options' => $eMainCountries, 'label' => false, 'class' => 'formstyler countrySelect'))?>
						</div>
					</div>
...
//        $('.progress').css('height', '0');
		...
		$('.bigCheckBox .jq-checkbox').click(function(){
			group_settings_flag = true;
		});
		...
		$('input[name*="data[Group][title]"], textarea[name*="data[Group][descr]"], input[name*="data[GroupAdministrator][role]"]').on('input', function(){
			group_settings_flag = true;
		});
		...
		function TestString(str, sLen) {
			return ( (str.length >= sLen) && (/[a-zA-Z]/.test(str) || /[�-��-�]/.test(str)) )
		}
	});
			...
			<div class="input-group input-large">
				<span class="input-group-addon glyphicon-extended glyphicons direction"></span>
								<select name="data[GroupAddress][{%=o.i%}][country]" class="formStyler countrySelect">
									<?php foreach($eMainCountries as $code=>$name): ?>
...
<div class="allCountries" style="display:none">
	<?php
	asort($eAllCountries);
	foreach($eAllCountries as $code=>$name){ ?>
		<option value="<?php echo $code?>"><?php echo $name;?></option>
	<?php }?>
</div>
