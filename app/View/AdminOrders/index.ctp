<?
	$title = __('Orders');
	$createTitle = __('Create order');
    $createURL = $this->Html->url(array('action' => 'edit', 0));
    $actions = $this->PHTableGrid->getDefaultActions('Order');
    $actions['table']['add']['href'] = $createURL;
    $actions['table']['add']['label'] = $createTitle;
    
    $backURL = $this->Html->url(array('action' => 'index'));
    $deleteURL = $this->Html->url(array('action' => 'delete'), true).'/{$id}?model=Order&backURL='.urlencode($backURL);
...
<br/>
<?
    echo $this->PHTableGrid->render('Order', compact('actions', 'columns'));
?>