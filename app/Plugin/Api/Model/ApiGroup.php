...
class ApiGroup extends AppModel {
	...
	*/
	public function getUserAdminGroups($userId){
		...
		$groupsId = array_keys($this->Group->userGroups($userId));

		return $this->formatGroupData($this->Group->find('all', array('conditions' => array('Group.id' => $groupsId))), $userId);
	...
	* @return array
	*/
	private function formatGroupData($data, $userId=false) {
		...
		foreach($data as $id=>$group){
			...
			$groupData['Group'][$id]['rating'] = $group['Group']['rating'];
			$groupData['Group'][$id]['is_crown'] = ($group['Group']['is_dream'] ? true : false);
			$groupData['Group'][$id]['is_owner'] = ($group['Group']['owner_id'] == $userId ? true : false);
		}
	...
	*/
	public function getInfo($groupId,$userId){
		...
		$fields = array('Group.id','Group.title','Group.descr as headline','Group.owner_id','Group.hidden','Group.cat_id','Group.video_url', 'Group.is_dream as is_crown', 'GroupMedia.*');
	
		$result = $this->Group->findById($groupId,$fields);
		if(!$result){
			return array();
		}
		$result['Group']['is_crown'] = (bool)$result['Group']['is_crown'];
	...
	*/
	...
	/**
	 * Отметить сообщество короной
	 *
	 * @param int $userId
	 * @param void $data
	 * @return void
	 */
	public function markGroupAsDream($userId, $data) {
		if(!$this->isAdmin($data['group_id'], $userId)) {
			throw new ApiIncorrectRequestException();
		}
		$result = $this->Group->updateAll(
				array('Group.is_dream' => ($data['is_dream'] ? 1 : 0)),
				array('Group.id' => $data['group_id'], 'Group.owner_id'=>$userId)
		);
		if(!$result)
			throw new Exception('Server Error');
