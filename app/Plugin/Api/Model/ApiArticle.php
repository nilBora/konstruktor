...
class ApiArticle extends AppModel {
	...
	*/
	public function usersArticles($userId){
		...
		$articles = $this->Article->find('all', compact('conditions', 'order'));
		...
		foreach ($articles as $id=>$item){
			...
			$aResult['Article'][$id]['category_title'] = $categoty[$item['Article']['cat_id']];
	...
	* @return array
	*/
	public function categoryArticles($userId,$categoryId, $allArt=false){
		$this->loadModel('Api.ApiArticleEvent');
	...
	*/
	public function getArticleBody($articleId){
		...
		$this->loadModel('ArticleCategory');
		$categoryOptions = $this->ArticleCategory->options();
		...
		$result['Article']['category_title'] = $categoryOptions[$data['Article']['cat_id']];
