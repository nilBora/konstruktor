...
class MaintenanceComponent extends Component {
	...
	public function checkMaintenance(){
		...
		if ($maintenanceEnable === true) {
			// maintenance status
			if (!$this->isAllowedAction($this->controller->request->params)
				&& strstr(Router::url('', true), $this->controller->request->webroot)
				&& Router::url('', true) != Router::url($maintenanceUrl, true)
				&& !in_array($clientIp, (array)$this->allowedIp)) {
				$this->controller->redirect($maintenanceUrl, $this->redirectStatus);
			}
		} elseif($this->controller->request->params['controller'] == $this->maintenanceUrl['controller']) {
