...
$(document).ready(function(){
	...
	$(document).on('click', '#open-user-list-panel', function(){
		if(animationEnd){
			animationEnd = false;
			if(!$(this).hasClass('active')){
				...
				$('.user-list-windpw-minimize').animate({opacity:1, top:205},500,function(){
					animationEnd = true;
				});
				click = true;
...
var UserList = (function () {
	function UserList(jQuery, options) {
		...
		$(document).on('click','.jcarousel-control-prev',function(){
			_this.thisScroll.scrollBy(0,45,300);
		});
	...
	UserList.prototype.addDroppable = function () {
		$('.dropme-files img').droppable({
			...
			drop: function( event, ui ) {
				...
				$.post('/ChatAjax/sendTimelineFiles.json', {fileId: fileId, roomId: roomId, userInitial: userInitial, userRecipient: userRecipient}, ( function (data) {
		...
		$('body').find('.user-list-item').droppable({
			...
			drop: function( event, ui ) {
				...
				setTimeout(function() {
					...
					$('.event-select').on("transitionend webkitTransitionEnd oTransitionEnd MSTransitionEnd", function(event){
						if (event.originalEvent.propertyName == 'opacity' && eventType != 'document') {
							Timeline.showEventPopup(sql_date, hours, action, side, eventType);
						}
					});
	...
	UserList.prototype.populateList = function (rawUserList) {
		...
		if ((rawUserListCount != chatMembersOldCount)) {
			...
			if($('.jcarousel-control').length == 0)
				$('<a href="#" class="jcarousel-control jcarousel-control-prev inactive" data-jcarouselcontrol="true">‹</a><a href="#" class="jcarousel-control-next jcarousel-control inactive" data-jcarouselcontrol="true">›</a>').insertAfter(this.$el);
			if (userList.length == 0) {
				...
				for (var i = 0; i < userList.length; i++) {
					...
					if(userList[i].members == 1){
						...
						$("<img/>").addClass("profile-picture rounded "+userList[i].ProfilePictureClass).attr("src", userList[i].ProfilePictureUrl).appendTo($userListItem);
